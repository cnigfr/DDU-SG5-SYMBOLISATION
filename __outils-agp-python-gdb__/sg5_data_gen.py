...
#out_folder_path = r"C:\Users\n.kulpinski\OneDrive - Aix Marseille Provence Métropole\Projet\CNIG_DDU\SG5-SYMBOLISATION-SYMBOLISATION\PLU\AGP\script"
...
while ligne!='' : # soit tant que la ligne n'est pas vide
    ...
    cursor = arcpy.da.InsertCursor(plu_zone_urba_carreau,["SHAPE@","typzone","blanc","etiquette"])
    ...
    cursor.insertRow([polygon,typzone,blanc,typzone])
...
while ligne!='' : # soit tant que la ligne n'est pas vide
    ...
    lin = ligne_separee [6] 
    symb_lin = ligne_separee [7] 
    pt = ligne_separee [8] 
    symb_pct = ligne_separee [9] 
    etiquette = f"{ligne_separee [10]} ({surf}{lin}{pt})"
    classement = ligne_separee [11]
    if classement != "sous-code" :
        ...
        cursor = arcpy.da.InsertCursor(plu_psc_carreau,["SHAPE@","typepsc","stypepsc","nnn","lib_stype","blanc","etiquette"])
        carreau = arcpy.Array([arcpy.Point(x - 10, y - 10),arcpy.Point(x - 10, y + 80),arcpy.Point(x + 80, y + 80),arcpy.Point(x + 80, y - 10)])
        polygon = arcpy.Polygon(carreau, spatial_reference)
        cursor.insertRow([polygon,typepsc,stypepsc,nnn,lib_stype,blanc,etiquette])
        if surf == "S" :
...
while ligne!='' : # soit tant que la ligne n'est pas vide
    ...
    if classement != "sous-code" :
        ...
        cursor = arcpy.da.InsertCursor(plu_inf_carreau,["SHAPE@","typeinf","stypeinf","nnn","lib_stype","blanc","etiquette"])
        carreau = arcpy.Array([arcpy.Point(x - 10, y - 10),arcpy.Point(x - 10, y + 80),arcpy.Point(x + 80, y + 80),arcpy.Point(x + 80, y - 10)])
        polygon = arcpy.Polygon(carreau, spatial_reference)
        cursor.insertRow([polygon,typeinf,stypeinf,nnn,lib_stype,blanc,etiquette])
        if surf == "S" :
...
while ligne!='' : # soit tant que la ligne n'est pas vide
    ...
    cursor = arcpy.da.InsertCursor(psmv_zone_urba_carreau,["SHAPE@","typzone","plu_psmv","blanc","etiquette"])
    carreau = arcpy.Array([arcpy.Point(x - 10, y - 10),arcpy.Point(x - 10, y + 80),arcpy.Point(x + 80, y + 80),arcpy.Point(x + 80, y - 10)])
    polygon = arcpy.Polygon(carreau, spatial_reference)
    cursor.insertRow([polygon,typzone,plu_psmv,blanc,typzone])
...
while ligne!='' : # soit tant que la ligne n'est pas vide
    ...
    symb_pct = ligne_separee [9] 
    etiquette = f"{ligne_separee [10]} ({surf}{lin}{pt})"
    plu_psmv = ligne_separee [11]
    classement = ligne_separee [12]
    if classement != "sous-code" :
        print(f"#"*20)
        ...
        cursor = arcpy.da.InsertCursor(psmv_psc_carreau,["SHAPE@","typepsc","stypepsc","nnn","lib_stype","plu_psmv","blanc","etiquette"])
        carreau = arcpy.Array([arcpy.Point(x - 10, y - 10),arcpy.Point(x - 10, y + 80),arcpy.Point(x + 80, y + 80),arcpy.Point(x + 80, y - 10)])
        polygon = arcpy.Polygon(carreau, spatial_reference)
        cursor.insertRow([polygon,typepsc,stypepsc,nnn,lib_stype,plu_psmv,blanc,etiquette])
        if surf == "S" :
...
while ligne!='' : # soit tant que la ligne n'est pas vide
    ...
    if classement != "sous-code" :
        ...
        cursor = arcpy.da.InsertCursor(psmv_inf_carreau,["SHAPE@","typeinf","stypeinf","nnn","lib_stype","plu_psmv","blanc","etiquette"])
        carreau = arcpy.Array([arcpy.Point(x - 10, y - 10),arcpy.Point(x - 10, y + 80),arcpy.Point(x + 80, y + 80),arcpy.Point(x + 80, y - 10)])
        polygon = arcpy.Polygon(carreau, spatial_reference)
        cursor.insertRow([polygon,typeinf,stypeinf,nnn,lib_stype,plu_psmv,blanc,etiquette])
        if surf == "S" :
